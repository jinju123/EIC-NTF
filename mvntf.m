function [t]=mvntf(X,R,L,A,B,C,options,realEndmember)
...
objNew=norm(X1-A*myKr(C,B,ones(1,R),ones(1,R)*L)','fro');%min_{A}||X_{JK×I}?(B@C) · AT||2 F
btdRes{1}=A;
btdRes{2}=B;
btdRes{3}=C;
...
iter=1;
%intial W
% X_noise=reshape(X,I*J,K);
% [A_init,~] = hyperVca(X_noise',R);
% % use FCLS to initiate S_init
% S_init = fcls(A_init,X_noise');
% Ssparse=X_noise'-A_init*S_init;
%Ssparse=rand(100,100*198);
%[~,num_end]=size(C_VCA);
%[D,Wc]=updateDW(C_VCA);
%Wc=D;
while 1
    ...
    %Aold=A;
    A=A.*((X1*M1+derta*sumToOne*B)./(A*M1'*M1+1e-4+derta*A*B'*B));
...
%        for ii=1:R
%         Ci_average=sum(C(:,ii))/K;
%         W1(:,ii)=Ci_average./C(:,ii);
%        end
%        for ii=1:R
%         Ci_max=norm(C(:,ii),'fro');
%         W1(ii)=Ci_max;
%        end
   % dvC=dv(C);
   % W1=1./(dvC+eps);
...
%      Ww1(1:R,:)=diag(W1);
%     Ww1=C*Wd1;
    C=C.*((X3*M3)./((C*M3'*M3)+lambda*W1));
    %[D,Wc]=updateDW(C);
    %Ssparse=Ssparse.*(X1-A*M1')./(Ssparse+eps);
%      [C] = derictional_tv(M3,X3,'MU',0.01,'POSITIVITY','yes', ...
%                                  'LAMBDA_TV',lambda, 'TV_TYPE','niso',...
%                                  'IM_SIZE',[100,100],'AL_ITERS',200, 'VERBOSE','yes');%代码有问题
%      [C] = direction_tv(M3,X3,'MU',0.01,'POSITIVITY','yes', ...
%                                  'LAMBDA_TV',lambda, 'TV_TYPE','niso',...
%                                  'IM_SIZE',[198,4],'AL_ITERS',200, 'VERBOSE','yes','X0',C);              
